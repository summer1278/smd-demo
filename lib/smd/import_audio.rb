...
class ImportAudio
  ...
  def generate_cfa_data 
    feature_plan = 'yaafe.py -r 11025 --resample -f "cfa: SimpleNoiseGate>ContinuousFrequencyActivation BinThreshold=10 NbPeaks=40>WindowConvolution WCLength=17" -p Metadata=False'
    system(feature_plan + ' '+ @output_directory+'/'+@uuid+'.mp3') #base dir can be set, name?
  end
